...
class SyncWritMonitoringToGoogleCalendar implements ShouldQueue
{
    ...
    public function handle(GoogleCalendarService $googleCalendar): void
    {
        ...
        if (! $writ || ! $writ->monitoring_at) {
            return;
        }

        $googleCalendar->syncWritMonitoring($writ);
    }
    ...
    public function failed(Throwable $exception): void
    {
        Writ::query()
            ...
            ->update([
                'google_calendar_monitoring_sync_error' => Str::limit($exception->getMessage(), 2000, ''),
            ]);
    }
}
